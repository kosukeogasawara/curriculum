...
public abstract class BaseServlet extends HttpServlet {
    ...
    protected boolean validateSession(HttpServletRequest request, HttpServletResponse response) throws IOException, MVCException {
        ...
        boolean hasSession = false;//blloean型の変数hassessionを定義
    ...
    protected boolean validateLogin(final HttpServletRequest request) throws IOException {
        ...
        try {
            ...
            // 最初の1件を取得
            Logger.log(new Throwable(), "reqStatus  = " + responseBean.getEmplyeeBeanList().stream().findFirst());
        ...
        } catch (MVCException e) {//MVCException：コントローラーの処理中におこる例外を処理するもの
            Logger.log(e);
