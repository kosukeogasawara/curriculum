    ...
    private void executeSelectQuery(final ExecuteCase eCase, final List<EmployeeBean> pEmployeeBeanList)
        ...
        try {
            ...
            case FIND_BY_EMPID:
                ...
                // [ここへ記述]
                for(EmployeeBean employee : pEmployeeBeanList) {
                    emp = employee;//for (要素の型 要素変数 : コレクションまたは配列){ループの中身}
                    break;
                }
                ...
                if (Objects.nonNull(emp)) {
                    ...
                    // [ここへ記述]
                    PreparedStatement psExecuteUpdate = connection.prepareStatement(ConstSQL.SELECT_BY_EMPID);
